...
class Pipeline(nn.Module):
    ...
    def heuristic_op(self, config, actions, obj_fn, k, destory_p=[0.5]):
        """
        - input: actions (L, N, F)
        ...
        new_actions_list = []
        # for p in destory_p:
        #     for i in range(k//len(destory_p)):
        #         shuffled_indices = self.destory_repair_index(2 * num_per_team, p)
        #         new_actions = []
        #         for j in range(actions.shape[0]):
        #             #(num_matches, 2 * num_per_match, N, F)
        #             new_action = actions[j].index_select(dim=0, index=torch.LongTensor(shuffled_indices).to(device))
        #             new_actions.append(new_action)
        #         new_actions_list.append(torch.stack(new_actions, dim=0))
        # for p in destory_p:
        #     for i in range(k//len(destory_p)):
        #         shuffled_indices = self.destory_repair_index(2 * num_per_team, p)
        #         new_action = actions.index_select(dim=1, index=torch.LongTensor(shuffled_indices).to(device))
        #         new_actions_list.append(new_action)
        # actions (num_matches, 2 * num_per_team,  batch_size)
        # shuffled_indices_list (k, 2 * num_per_team)
        # (k, num_matches, 2 * num_per_team,  batch_size) (k, num_matches, 2 * num_per_team, batch_size)
        new_actions = actions.unsqueeze(0).repeat(k,1,1,1).gather(dim=2, index=self.shuffled_indices_list[:,None,:,None].expand(-1,num_matches,-1,batch_size))
        # print('new_actions ', new_actions.shape, batch_size, k, num_matches)
        k_actions = new_actions
        # k_actions = torch.stack(new_actions_list, dim=0) #(k, num_matches, 2 * num_per_match, batch_size, F)
        ...
        y_obj = [obj_fn(match.reshape(-1,2,num_per_team)) for match in y]
        ...
        # a_min_2d = a_min.unsqueeze(-1).unsqueeze(1).expand(ori_actions.shape)
        #
        # result = torch.gather(ori_actions, 1, a_min_2d) # (1,batch_size, num_matches, 2 * num_per_team)
        # result = torch.gather(ori_actions, dim=1, index=a_min.unsqueeze(-1).repeat(1, 1, 2 * num_per_team))
        result = torch.gather(ori_actions, dim=1, index=a_min[:, None,:, None].expand(-1, 1, -1, ori_actions.size(3))).squeeze(1)
        # print(result.shape)
        # print(a, k_actions.permute(3, 0, 1, 2)[-1][a_min[-1][-1]][-1], result[-1,-1,:])
        return result.permute(1,2,0).reshape(-1,batch_size,1)
    ...
    def fit(self, train_ds, valid_ds, num_epochs=1, batch_size=128):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step, batch in enumerate(train_loader):
                ...
                new_actions = self.heuristic_op(self.conf, actions, SimpleMatchRecEnv.obj_fn, k=pow(self.conf['num_per_team'],3), destory_p=[0.4, 0.8])

                # batch_size, seq_len = states.size(0), states.size(2)
                env_steps += 1
                R = (self.reward_fn(actions) + self.reward_fn(new_actions))/2
                # print(actions[:,0], self.reward_fn(actions)[0])
                # print(new_actions[:,0], self.reward_fn(new_actions)[0])
    ...
    def evaluate(self, test_loader):
        ...
        for batch in test_loader:
            ...
            actions, action_probas, indices = self.forward(states)
            new_actions = self.heuristic_op(self.conf, actions, SimpleMatchRecEnv.obj_fn, k=pow(self.conf['num_per_team'],3), destory_p=[0.4, 0.8])
            # new_actions = self.heuristic_op(self.conf, actions, SimpleMatchRecEnv.obj_fn, k=100, destory_p=[0.4, 0.8])
            R = self.reward_fn(actions).mean().item()
            reward.append(R)
            new_R = self.reward_fn(new_actions).mean().item()
            new_reward.append(min(new_R, R))

        # print('eval', reward, new_reward)
        # print('eval', R, new_R)
        # print(actions[:,-1,0], new_actions[:,-1,0])
        # raise Exception
        return np.array(new_reward).mean()
