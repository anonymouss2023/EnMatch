...
import sys
import json
# from enmatch.env.slate import SlateRecEnv, SlateState
# from enmatch.env.seqslate import SeqSlateRecEnv, SeqSlateState
from enmatch.env.simpleMatch import SimpleMatchRecEnv, SimpleMatchState
from enmatch.env.seqSimpleMatch import SeqSimpleMatchRecEnv, SeqSimpleMatchState
import http.client
...
class Envs(object):
    ...
    def create(self, env_id, config={}, seed=None):
        ...
        try:
            if env_id == 'SimpleMatchRecEnv-v0':
                config['gpu'] = False
                sim = SimpleMatchRecEnv(config, state_cls=SimpleMatchState)
                env = gym.make('SimpleMatchRecEnv-v0', recsim=sim)
            elif env_id == 'SeqSimpleMatchRecEnv-v0':
                config['gpu'] = False
                sim = SeqSimpleMatchRecEnv(config, state_cls=SeqSimpleMatchState)
                env = gym.make('SeqSimpleMatchRecEnv-v0', recsim=sim)
            else:
                env = gym.make(env_id)
            if seed:
                env.seed(seed)
