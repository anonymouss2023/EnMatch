...
class RecDataBase(object):
    ...
    def __init__(self, config, state_cls):
        ...
        if self.is_eval:
            self.cache_size = config.get('cache_size', self.batch_size)
        else:
...
class RecSimBase(ABC):
    ...
    def __init__(self, config, state_cls):
        ...
        if model_file and len(model_file)>1:
    ...
    def __init__(self, recsim: RecSimBase):
        ...
        if self.config.get("rawstate_as_obs", False):
            ...
            if self.config.get("support_rllib_mask", False):
                ...
                self.observation_space = gym.spaces.Dict({
                    "action_mask": gym.spaces.Box(0, 1, shape=(action_feature_size,)),
                    "obs": gym.spaces.Box(-100000000.0, 100000000.0, shape=(len(self.obs[0]["obs"]),))
                })
            else:
                self.observation_space = gym.spaces.Box(-100000000.0, 100000000.0, shape=(len(self.obs[0]),))
